...
export default function CalendarPage() {
  ...
  return (
    <div>
      <h1 className="mb-6 text-2xl font-bold text-[var(--slate-800)]">Calendar</h1>

      <div className="glass-card p-6">
        <div className="mb-6 flex items-center justify-between">
          <button onClick={prevMonth} className="rounded-lg p-2 hover:bg-[var(--slate-50)]">
            <ChevronLeft className="h-5 w-5 text-[var(--slate-500)]" />
          </button>
          <h2 className="text-lg font-semibold text-[var(--slate-800)]">{monthName}</h2>
          <button onClick={nextMonth} className="rounded-lg p-2 hover:bg-[var(--slate-50)]">
